...
var allowedSyscalls = seccomp.SyscallRules{
	...
	unix.SYS_RECVMSG: {
		...
		},
		{
			seccomp.MatchAny{},
			seccomp.MatchAny{},
			// mdlayher/netlink peeks the complete datagram length before
			// allocating its receive buffer.
	...
	unix.SYS_SETSOCKOPT: {
		...
		},
		{
			seccomp.MatchAny{},
			seccomp.EqualTo(unix.SOL_SOCKET),
			seccomp.EqualTo(unix.SO_RCVBUFFORCE),
			seccomp.MatchAny{},
			seccomp.EqualTo(4),
		},
		{
		...
		},
		{
			seccomp.MatchAny{},
			seccomp.EqualTo(unix.SOL_SOCKET),
			seccomp.EqualTo(unix.SO_SNDBUFFORCE),
			seccomp.MatchAny{},
			seccomp.EqualTo(4),
		},
		{
